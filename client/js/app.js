...
class EventsManager {
    ...
    obtenerDataInicial() {
        ...
        $.ajax({
          ...
          success: (data) =>{
            if (data.msg=="OK") {
              //var evento = "[{title: '"+data.eventos.title+"', start: '"+data.eventos.star+"'}]";
          ...
          error: function(){
            alert("error en la comunicación con el servidor main");
          }
        })
    ...
    eliminarEvento(event, jsEvent){
      ...
      $.ajax({
        ...
        success: (data) =>{
          if (data.msg=="OK") {
            alert('Se ha eliminado el evento exitosamente '+event.title)
          }else {
            alert(data.msg)
          }
        },
